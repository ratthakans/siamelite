...
> จุดเด่น และอัลบั้มรูป — แก้ได้ครบผ่าน CMS ที่ `/admin`
...
**เปิดให้ login แก้ไขจากที่ไหนก็ได้ (production) — ใช้แค่ Vercel + GitHub ไม่ต้องมี Netlify:**

ตัวกลาง OAuth (`api/auth.js` + `api/callback.js`) เขียนเป็น Vercel serverless function เอง
deploy พร้อมเว็บอยู่แล้ว (ไม่ต้อง host แยก) เหลือแค่:
...
1. สร้าง GitHub OAuth App ที่ https://github.com/settings/developers → **New OAuth App**
   ...
   - Authorization callback URL: `https://siamelite.vercel.app/api/callback` **(ต้องเป๊ะตรงนี้)**
   - กด Register แล้วคัดลอก **Client ID** และกด "Generate a new client secret" คัดลอก **Client Secret**
2. ไปที่ Vercel dashboard → โปรเจกต์ `siamelite` → **Settings → Environment Variables**
   เพิ่ม 2 ตัว (Production + Preview):
   - `GITHUB_OAUTH_CLIENT_ID` = Client ID จากข้อ 1
   - `GITHUB_OAUTH_CLIENT_SECRET` = Client Secret จากข้อ 1
   แล้ว redeploy 1 ครั้ง (Deployments → ⋯ → Redeploy) เพื่อให้ env vars มีผล
...
5. **Google Map** — ใน section About ตอนนี้ฝังแผนที่ Google ชี้ไปย่านนิมมานเป็นค่าเริ่มต้น
   แก้ `src` ของ `<iframe>` ใน `index.html` (`.about-media`) ให้ชี้ที่อยู่สำนักงานจริงของคุณ
   (เปิด Google Maps → ปักหมุดที่อยู่ → Share → Embed a map → คัดลอก URL มาใส่)
6. **รีวิวลูกค้า** — เปลี่ยนข้อความ testimonial เป็นรีวิวจริง (ยิ่งมีรูปคู่ทีมงานยิ่งดี)
7. **ทีมงาน (Team section ใหม่)** — ใน `index.html` ส่วน `id="team"` มีการ์ด 3 ใบเป็น placeholder
   ("ใส่ชื่อจริง") ใส่ชื่อ-รูปถ่ายทีมจริง เพิ่มความน่าเชื่อถือมาก (แทน `SE` ใน `.team-av` ด้วย
   `<img src="...">`)
8. **Analytics** — ใน `assets/js/app.js` ด้านบนมี `GA4_ID` และ `META_PIXEL_ID` เป็น placeholder
...
> ⚠️ มี **lead ทดสอบ 2 รายการ** ในตาราง Supabase (`ทดสอบระบบ CMS`, `Consent Test`) จากการเทสต์
